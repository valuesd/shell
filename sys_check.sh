...
sys_check_file="logs/$(ip a show dev ens33|grep -w inet|awk '{print $2}'|awk -F '/' '{print $1}')-`date +%Y%m%d`.txt"
...
function get_net_info() {
    pri_ipadd=$(ip a show dev ens33|grep -w inet|awk '{print $2}'|awk -F '/' '{print $1}')
